...
bool is_irreducible(const Field& field, const Polynomial& modulus) {
    ...
    // first, as the original's prompt did.
...
std::vector<Matrix> reduce_tensor_modulo(const Field& field, std::vector<Matrix> slices,
    ...
    // The remainder is what is left below the modulus's degree; the original
    // dropped every zero coefficient, which is what makes the tensor shrink.
