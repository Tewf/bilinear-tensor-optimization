...
Notation and the cost of each primitive: [`../exact/README.md`](../exact/README.md).
...
nothing it walks `Σ_s C(b,s)` of them on the way down. This is the algorithm
the original could never run: both arms of the choice offering it called the
bottom-up version instead.

## The caveat

These count **field operations**. Over `Q` those are not constant time — see
[the exact layer](../exact/README.md#the-caveat-that-matters-not-all-field-operations-cost-the-same).
