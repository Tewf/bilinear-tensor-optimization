...
Matrix row_basis_sparsifier(const Field& field, const Matrix& u) {
    ...
    std::size_t fewest = exact::nonzero_count(field, u);

    for (const std::vector<std::size_t>& chosen : subsets(u.rows(), order)) {
        const Matrix block = exact::select_rows<Field>(u, chosen);
        Matrix inverse;
        if (!exact::invert(field, block, inverse)) continue;

        const std::size_t count = exact::nonzero_count(field, exact::multiply(field, u, inverse));
        if (count < fewest) {
            fewest = count;
            best = inverse;
        }
...
Validator find_validator(const Field& field, const Matrix& rows,
    ...
    const Matrix restricted = exact::select_columns<Field>(rows, columns);
    ...
    for (std::size_t candidate = 0; candidate < rows.rows(); ++candidate) {
        ...
        if (!exact::solve_in_row_space(field, others, restricted.row(candidate), coefficients)) {
            continue;
        }
...
Matrix sparsify_exhaustive(const Field& field, Matrix rows) {
    ...
    for (const std::vector<std::size_t>& columns : subsets(rows.columns(), rows.rows() - 1)) {
        if (exact::rank(field, exact::select_columns<Field>(rows, columns)) == rows.rows() - 1) {
