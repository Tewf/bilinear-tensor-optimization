...
void check_decomposition(const exact::ModularField& field, const exact::ModularMatrix& matrix,
                         const std::string& what) {
    const std::size_t expected_terms = exact::rank(field, matrix);
    const std::vector<exact::ModularMatrix> terms = exact::rank_one_decomposition(field, matrix);
    ...
    exact::ModularMatrix total(matrix.rows(), matrix.columns());
    for (const exact::ModularMatrix& term : terms) {
...
int main(int argc, char** argv) {
    ...
    for (int64_t characteristic : {2, 3, 5, 7, 13}) {
        const exact::ModularField field(characteristic);
        ...
        for (int trial = 0; trial < 40; ++trial) {
            ...
            exact::ModularMatrix matrix(rows, columns);
