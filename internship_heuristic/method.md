...
Julia's default sort is not stable, so the original left this to whatever
quicksort did; pinning it is what makes the published ranks reproducible.
...
both exclusions in
[`tests/test_exhaustive_search.cpp`](../exhaustive_search/tests/test_exhaustive_search.cpp), with the
11 as its own `slow`-labelled test, and GF(8)'s 6 in
[`tests/test_map_construction.cpp`](../map_construction/tests/test_map_construction.cpp) beside the
