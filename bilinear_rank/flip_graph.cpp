...
bool same(const Field& field, const std::vector<Element>& left,
          const std::vector<Element>& right) {
    if (left.size() != right.size()) return false;
    for (std::size_t index = 0; index < left.size(); ++index) {
        if (!field.areEqual(left[index], right[index])) return false;
    }
    return true;
}
        ...
        for (std::size_t first = 0; first < scheme.size() && !again; ++first) {
            for (std::size_t second = first + 1; second < scheme.size() && !again; ++second) {
                for (const Mode shared : {kLeft, kRight, kOutput}) {
                    ...
                    if (!same(field, part(scheme[first], shared), part(scheme[second], shared)) ||
                        !same(field, part(scheme[first], other), part(scheme[second], other))) {
                        continue;
                    }
...
Scheme walk(const Field& field, Scheme scheme, std::size_t steps, std::uint64_t seed,
    ...
    for (std::size_t step = 0; step < steps && scheme.size() > 1; ++step) {
        // Every pair that shares a factor, in any mode: those are the flips
        // available from here.
        ...
        for (std::size_t first = 0; first < scheme.size(); ++first) {
            for (std::size_t second = 0; second < scheme.size(); ++second) {
                ...
                for (const Mode shared : {kLeft, kRight, kOutput}) {
                    if (same(field, part(scheme[first], shared), part(scheme[second], shared))) {
                        moves.push_back({first, second, static_cast<std::size_t>(shared)});
                    }
                }
            }
        }
        ...
        const auto [first, second, shared_mode] =
            moves[std::uniform_int_distribution<std::size_t>(0, moves.size() - 1)(generator)];
        const Mode shared = static_cast<Mode>(shared_mode);
        const auto [other, third] = others(shared);

        // A⊗B⊗Γ + A⊗B'⊗Γ' = A⊗(B+B')⊗Γ + A⊗B'⊗(Γ'−Γ)
        const std::vector<Element> was_third = part(scheme[first], third);
        add_into(field, part(scheme[first], other), part(scheme[second], other));
        subtract_from(field, part(scheme[second], third), was_third);
        ++flips;
