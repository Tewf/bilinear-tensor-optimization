...
linear_algebra::SpanBasis<Field> span_of(const Field& field, std::size_t width,
...
std::vector<Matrix> rank_one_maps_within_span(const Field& field,
                                              const linear_algebra::SpanBasis<Field>& reachable,
                                              std::size_t width, const std::vector<Matrix>& pool,
                                              std::size_t needed,
                                              std::vector<Element>& scratch) {
    std::vector<Matrix> found;
    linear_algebra::SpanBasis<Field> independent(field, width);
...
std::vector<Matrix> rank_one_maps_within(const Field& field, const std::vector<Matrix>& subspace,
    ...
    const std::size_t width = entry_width(subspace);
    std::vector<Element> scratch;
    return rank_one_maps_within_span(field, span_of(field, width, subspace), width, pool,
                                     pool.size(), scratch);
}
...
bool expand_subspace_impl(const Field& field, linear_algebra::SpanBasis<Field> span,
    ...
    for (std::size_t index = from; index < pool.size(); ++index) {
        ...
        linear_algebra::SpanBasis<Field> extended = span;
...
bool expand_subspace(const Field& field, const std::vector<Matrix>& subspace,
    ...
    const std::size_t width = entry_width(subspace);
    std::vector<Element> scratch;
    return expand_subspace_impl(field, span_of(field, width, subspace), width, pool, from, target,
...
bool fewest_products_by_sweep(const Field& field, const std::vector<Matrix>& base,
    ...
    const std::size_t width = entry_width(base);
    const std::size_t lowest = span_of(field, width, base).dimension();
...
bool fewest_products_by_bisection(const Field& field, const std::vector<Matrix>& base,
    ...
    if (base.empty()) return false;
    const std::size_t width = entry_width(base);
    std::size_t low = span_of(field, width, base).dimension();
