...
/// rank one.
///
/// The original read the right-hand operand from row 0 while computing the
/// index of the first nonzero row and using it for everything else, so any
/// slice whose first row was zero produced a zero operand row and an algorithm
/// that did not compute the map.
