...
Verdict solve_with(const satisfiability::BinaryEncoding& encoding, const std::vector<int>& cube) {
    linear_algebra::Cnf formula = encoding.formula;
    for (const int literal : cube) formula.add_clause({literal});

    const satisfiability::SolverRun run = satisfiability::solve(formula, true, 2048, 120);
...
Verdict solve_over_cubes(const satisfiability::BinaryEncoding& encoding,
                         const std::vector<std::vector<int>>& cubes) {
    bool anything_unknown = false;
    for (const std::vector<int>& cube : cubes) {
        const Verdict verdict = solve_with(encoding, cube);
...
void check_cubes_do_not_change_the_answer(const bilinear_rank::Field& field, std::size_t products,
                                          Verdict expected) {
    ...
    const Verdict whole = solve_with(encoding, {});
    const Verdict split = solve_over_cubes(encoding, cubes);
...
int main(int argc, char** argv) {
    ...
    if (satisfiability::find_solver().empty()) {
        std::cout << "  skip  cubes against the whole formula: no solver on PATH\n";
    } else {
        check_cubes_do_not_change_the_answer(over_two, 7, Verdict::Yes);
        check_cubes_do_not_change_the_answer(over_two, 6, Verdict::No);
    }
